...
- **`decide_digit` keeps `prev_digit` in the stable zone** (`thr_dn < progress < thr_up`). It only flips to a new value at the thresholds. Changing `rolling_threshold_up/down` shifts where OCR flips vs. where the previous value is trusted. See [watermeter.py:565](watermeter.py:565).
